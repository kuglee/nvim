-- from: https://www.reddit.com/r/neovim/comments/1lm00a6/oilnvim_with_yazi_like_parentpreview_windows

-- from: https://gitlab.com/configs_and_scripts/nvim/-/blob/ad59c40a30efca4e2a5f11ce809dd1628ebcd9e1/config/lua/functions/helper_funcs.lua
...
-- from: https://gitlab.com/configs_and_scripts/nvim/-/blob/ad59c40a30efca4e2a5f11ce809dd1628ebcd9e1/config/lua/plugins/oil.lua
...
oil_ui.update_oil_paths = function(current, parent, preview)
  ...
  if
    join(current, "/") ~= join(vim.g.oil_state.current.window_path, "/")
    and join(current, "/") ~= ""
  then
    if
      tonumber(
        vim.fn.system("if [ -f '" .. join(current, "/") .. "' ]; then echo 1; else echo 0; fi")
      ) == 1
    then
      current =
        split_str(string.gsub(join({ unpack(current, 1, (#current - 1)) }, "/"), "[\n\r]", ""), "/")
    end
  ...
  if join(preview.window_path, "/") ~= join(vim.g.oil_state.preview.window_path, "/") then
    if join(preview.window_path, "/") ~= join(current.window_path, "/") then
      ...
      if
        tonumber(
          vim.fn.system(
            "if [ -f '" .. join(preview.window_path, "/") .. "' ]; then echo 1; else echo 0; fi"
          )
        ) == 1
      then
    ...
    oil_ui.preview_entry_update_timer:start(100, 100, function()
      if vim.g.oil_preview_active then
        vim.schedule(function()
          if
            vim.fn.win_getid() == vim.g.oil_state.current.window_number and vim.fn.mode() == "n"
          then
...
oil_ui.cursor_file_preview = function()
  ...
  if selected_file ~= nil and selected_file.name ~= ".." then
    ...
    if join(current_dir, "/") ~= join(vim.g.oil_state.preview.window_path, "/") then
      ...
      _, _, new_preview =
        oil_ui.update_oil_buf_numbers(vim.g.oil_state.current, vim.g.oil_state.parent, new_preview)
...
oil_ui.close_oil_windows = function(file_to_open)
  ...
  local new_original_file = file_to_open ~= nil and file_to_open
    or vim.g.oil_state.original_file.path
...
oil_ui.select_oil_entry_with_parent_and_preview = function()
  ...
  if selected_file.name == ".." then
    ...
    if
      tonumber(
        vim.fn.system("if [ -f '" .. join(current_dir, "/") .. "' ]; then echo 1; else echo 0; fi")
      ) ~= 1
    then
      oil_ui.open_oil_with_parent_and_preview({
        window_number = vim.g.oil_state.current.window_number,
...
config.keymaps["<CR>"] = oil_ui.select_oil_entry_with_parent_and_preview
config.keymaps["l"] = { oil_ui.select_oil_entry_with_parent_and_preview, mode = "n" }
config.keymaps["h"] = {
  function()
...
config.keymaps["<C-c>"] = { oil_ui.close_oil_windows, mode = "n" }
config.keymaps["-"] = {
  function()
    local new_current = vim.g.oil_state.current
    new_current.window_path = { unpack(new_current.window_path, 1, #new_current.window_path - 1) }
